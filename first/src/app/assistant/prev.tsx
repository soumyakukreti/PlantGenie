...
export default function Assistant() {
  const [plantInfoSent, setPlantInfoSent] = useState(false);
  ...
  const handleSendMessage = (event) => {
    ...
    // const question = cardData
    //   ? `Plant: ${cardData.name}, Description: ${cardData.description}, Message: ${message}`
    //   : message;
    ...
    // if (plantInfo) {
    //   userMessage.text = `Plant: ${plantInfo.name}, Description: ${plantInfo.description}, Message: ${message}`;
    // }
    let question = message;

    if (plantInfo && !plantInfoSent) {
      question = `Plant: ${plantInfo.name}, Description: ${plantInfo.description}, Message: ${message}`;
      setPlantInfoSent(true);
    }
      ...
      <div className="mainbox flex flex-col items-center">
        <div className="chatbox w-2/3 rounded-xl border-2 min-h-70 p-10 m-5 flex flex-col items-center">
          <div className="chatcontent w-3/4 border-4 min-h-80 rounded-xl m-5">
            <ScrollArea className="w-full min-h-80 rounded-md border">
